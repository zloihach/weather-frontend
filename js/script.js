document.addEventListener('DOMContentLoaded', function() {
    const endpoint = 'http://46.19.66.189:3000/weather';
...
function fetchDataAndRenderCharts(endpoint) {
    fetch(endpoint)
        ...
        .then(dataArray => {
            ...
            updateChart('temperatureChart', 'Температура', '°C', labels, temperatureData, 'rgb(255, 99, 132)', [5, 5]);
            updateChart('humidityChart', 'Влажность', '%', labels, humidityData, 'rgb(54, 162, 235)');
            updateChart('lightChart', 'Освещенность', 'Люкс', labels, lightData, 'rgb(255, 206, 86)', [10, 5]);
            updateChart('pressureChart', 'Давление', 'мм рт. ст.', labels, pressureData, 'rgb(75, 192, 192)');
        })
        .catch(error => {
...
function updateChart(canvasId, label, unit, labels, dataValues, color, borderDash = []) {
        ...
        window.myCharts[canvasId] = new Chart(ctx, {
            ...
            options: {
                scales: {
                    ...
                    x: {
                        reverse: true, // Разворачиваем ось X (время)
                        title: {
                            display: true,
                            text: 'Время'
                        }
